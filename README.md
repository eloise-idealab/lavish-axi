...
- **Real-time streaming** - `lavish-axi stream <html-file>` is a push alternative to `poll`: it holds a live connection open and prints one NDJSON line per user message as it arrives, so the agent can handle each message individually (for example, fan out a subagent per message) instead of draining one batch and exiting. Re-run it if the connection drops - anything not yet delivered stays queued until delivery, though delivery consumes it, so messages the dropped stream already printed do not arrive again - or pass `--once` to stop after the first message.
