...
Lavish Editor is a CLI + local HTTP server that opens agent-generated HTML artifacts in a browser, lets the user annotate elements or selected text ranges, and ships those annotations back to the agent over a long-polling or real-time SSE streaming API.
...
It shuts itself down once no browser chrome (SSE) and no agent poll or stream have been connected for `LAVISH_AXI_IDLE_TIMEOUT_MS` (default 30 minutes; set `0`/`off` to disable), and immediately when the last open session ends while nothing is connected (a still-attached browser, poll, or stream defers cleanup to the idle timer instead).
...
6. User actions in the iframe `postMessage` to the chrome (queue prompts, request snapshot, end session).
   ...
   `queuePrompts` mints a server-owned `id` (a UUID via `newMessageId`) for each user message and drops any `reply_to` that does not point at a message already in this session's transcript, so message ids can't be forged or duplicated and threads can't be spoofed through the unauthenticated local API.
   ...
   The annotation card exposes explicit Cancel, Queue, and Send actions, and its textarea follows the same convention: Enter queues the annotation (equivalent to clicking "Queue"); Shift+Enter inserts a newline; Ctrl+Enter (Cmd+Enter on macOS) queues the annotation and immediately sends all queued prompts (equivalent to clicking "Send"), and the card shows a small hint for these shortcuts.
7. The chrome top bar exposes annotation mode as an `Annotate` switch and keeps editing actions in an overflow menu: the home-shortened artifact path with a copy affordance, reload artifact, copy DOM snapshot, and end session.
   ...
   The conversation panel renders root messages with Slack-style thread chips; opening a chip slides out a thread pane where the user replies to the whole thread or targets a specific message, agent replies live-route into the open thread (with a Back badge for activity elsewhere), and unread thread replies surface as solid-brass chips until seen.
...
9. `lavish-axi stream <file.html>` (`streamCommand`) hits `GET /api/stream` for real-time push delivery instead of one-batch-per-poll.
   The server holds the SSE connection open and emits a `ready` frame, then one `message` frame per queued user message as it arrives (layout warnings and non-message prompts ride along on the batch's first frame), an `ended` frame when the session ends, and `: keep-alive` heartbeats in between.
   The CLI prints one NDJSON line per frame on stdout so the agent can read line-by-line and fan out a subagent per message; only real user messages count toward `delivered`, while annotation- or layout-warning-only frames are still emitted but do not satisfy `--once`.
   `--once` delivers exactly one user message and requeues the rest of the batch (via `requeueFeedback`) so a single-shot harness never loses the tail, then closes.
   Concurrent streams are capped at `maxStreamClients` (default 16); past the cap the server replies `503 busy` with a `Retry-After` header.
   Delivery is deliver-then-restore: `takeFeedback` clears a batch atomically (so two consumers never double-deliver), and a detected disconnect mid-drain requeues the batch and wakes another consumer rather than writing to a dead socket and dropping it.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation - there's no in-memory cache. Every file-touching operation runs through a store-global promise-chain lock (`withLock`) so a concurrent stream drain (`takeFeedback`) and an always-on Send (`queuePrompts`) can't interleave their read-modify-write windows and drop or duplicate user messages. The lock is store-global rather than per-key because the whole file is the read/write unit, so two different keys would still clobber each other's slice on a shared write.
- Message ids and `reply_to` targets are server-owned. `queuePrompts` and `addAgentReply` mint a UUID (`newMessageId`) for each message and accept a `reply_to` only when it points at a message already in the session transcript, dropping anything forged. `requeueFeedback` restores a batch a stream took (`takeFeedback`) but couldn't deliver because the client dropped, so the next poll/stream still gets it: prompts prepend ahead of anything queued meanwhile, while layout warnings and `dom_snapshot` are restored only when nothing fresher has arrived (replace-semantics).
