...
Server self-shutdown keys off live connections (browser SSE and agent polls or streams), not session status, so the next `lavish-axi <file>` re-spawns a fresh server and adopts the session from `state.json` when it is still resumable; README's Server cleanup bullet owns the user-facing idle-timeout rules.
...
8. `lavish-axi stream <file.html>` (`streamCommand`) hits `GET /api/stream` for real-time push delivery instead of one batch per poll.
   The server holds the connection open and emits a `ready` frame, then one `message` frame per queued user message as it arrives; the CLI prints one NDJSON line per frame on stdout so the agent can read line-by-line and fan out per message.
   Delivery is deliver-then-restore: `takeFeedback` clears a batch atomically so two consumers never double-deliver, and a disconnect detected mid-drain requeues the undelivered remainder via `requeueFeedback` - prompts are prepended, while layout warnings and `dom_snapshot` restore only when nothing fresher arrived, because those are replace-semantics.
   `--once` delivers exactly one message and requeues the rest of the batch, so a single-shot harness never loses the tail.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation; there's no in-memory cache. Every file-touching operation runs through a store-global promise chain (`withLock`), because a stream drain (`takeFeedback`) and a concurrent always-on send (`queuePrompts`) would otherwise interleave their read-modify-write windows and drop or duplicate user messages. The lock is store-global rather than per-session because the whole file is the unit of read/write.
- Message ids and `reply_to` targets are server-owned: `queuePrompts` and `addAgentReply` mint a UUID (`newMessageId`) per message and accept a `reply_to` only when it points at a message already in that session's transcript, so the unauthenticated local API cannot forge or duplicate an id or spoof a thread.
